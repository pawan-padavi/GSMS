<?php
    ...
    if(mysqli_num_rows($result) > 0)
    {
        ...
        while ($row=mysqli_fetch_assoc($result)) {
            ...
            <td><img src='Assets/upload-images/{$row["p_img"]}'alt='image not found' height='60' width='40'></img></td>
            ...
            <td><input type='hidden' name='sc_name'id='sc_name' class='form-control w-100' value='{$row["sc_name"]}'>{$row["sc_name"]}</td>
            <td><input type='hidden' name='p_brand'id='p_brand' class='form-control w-100' value='{$row["p_brand"]}'>{$row["p_brand"]}</td>
            <td><input type='hidden' name='p_price'id='p_price' class='form-control w-100' value='{$row["p_price"]}'>{$row["p_price"]}&nbsp;{$row["c_format"]}</td>
